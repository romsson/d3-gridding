...
export default function (nodes, v) {
  ...
  const xDomainMax = _valueXmax === 0 ? 1 : _valueXmax;
  v.x.domain([0, xDomainMax]).range([margin(v, "left") + v.padding, v.size[0] - margin(v, "right") - v.padding]);
  ...
  // Handle case where all values are the same
  const yDomainMax = _valueYmax === 0 ? 1 : _valueYmax;
  v.y.domain([0, yDomainMax]).range([margin(v, "top") + v.padding, v.size[1] - margin(v, "bottom") - v.padding]);
